...
public class UIEventListener : MonoBehaviour
{
...
#if wp_modify
    Collider _cacheCollider;
    bool _isCacheCollider;
    Collider cacheCollider {
        get {
            if (!_isCacheCollider) {
                _cacheCollider = GetComponent<Collider>();
                _isCacheCollider = true;
            }
    ...
    void OnPress (bool isPressed)	{
        if (isColliderEnabled) {
            ...
            if (isPressed) {
                iu.game.PoolsUtil.SpawnSound(iu.game.MusicDef.music_ui_dianji);
            }
        }
    }
#else
...
#if wp_modify
    static public UIEventListener Get(GameObject go, bool playSound = true) {
